...
class FindBar(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.btn_close = QPushButton("Γ£ò")
...
class SymbolDialog(QDialog):
    def __init__(self, parent=None):
        ...
        symbols = [
            "ΓåÉ", "Γåæ", "ΓåÆ", "Γåô", "Γåö", "Γåò",
            "Γÿà", "Γÿå", "Γùï", "ΓùÅ", "Γûí", "Γûá",
            "Γÿ║", "Γÿ╣", "ΓÖÑ", "ΓÖª", "ΓÖú", "ΓÖá",
            "Γ£ô", "Γ£ù", "Γê₧", "Γëê", "Γëá", "Γëñ",
            "ΓëÑ", "┬▒", "├╖", "├ù", "┬░", "╧Ç",
            "╬⌐", "╬╝", "╬ú", "Γé¼", "┬ú", "┬Ñ",
            "┬⌐", "┬«", "Γäó", "┬º", "┬╢", "ΓÇá"
        ]
...
# Override paste event to handle image pasting - handled by class methods
        ...
        self.btn_close_toc = QPushButton("├ù")
    ...
    def setup_toolbar(self):
        ...
        if hasattr(self, 'custom_highlight_color'):
             ...
             self.btn_custom_hl.setText("≡ƒîê") 
        ...
        self.combo_list.addItems(["List", "ΓÇó", "1.", "A.", "I.", "Γÿæ", "╬⌐"]) # Shortened labels
    ...
    def update_format_ui(self):
        ...
        try:
            ...
            if block_text.startswith("ΓÿÉ ") or block_text.startswith("Γÿæ "):
        ...
        ">
            <tr>
                ...
                <td width="30" style="vertical-align: top; text-align: right; padding: 5px;">
                     <a href="action://delete_level" style="text-decoration: none; color: #888888; font-weight: bold; font-size: 14px; font-family: Arial;">Γ£ò</a>
                </td>
            </tr>
        </table>
        <br>
    ...
    def apply_level_1(self):
        ...
        box_html = (
            ...
            f'<a href="action://delete_level" style="text-decoration: none; color: #444444; font-weight: bold; font-size: 12px; font-family: Arial;">Γ£ò</a>'
            f'</td>'
            f'</tr>'
            f'</table>'
            f'<p></p>'
    ...
    def _editor_key_press(self, event):
        ...
        if event.key() == Qt.Key.Key_Return or event.key() == Qt.Key.Key_Enter:
            ...
            if block_text.startswith("ΓÿÉ ") or block_text.startswith("Γÿæ "):
                 ...
                 if block_text.strip() in ["ΓÿÉ", "Γÿæ"]:
                     ...
                     # Continue list
                     QTextEdit.keyPressEvent(self.editor, event)
                     self.editor.insertPlainText("ΓÿÉ ")
                     return

            # 2. Custom Symbol Auto-Continue
            # We check for our defined symbols
            symbols = ["ΓåÆ", "Γåæ", "Γåô", "Γåö", "Γåò", "Γÿà", "Γÿå", "Γùï", "ΓùÅ", "Γûí", "Γûá", "Γÿ║", "Γÿ╣", "ΓÖÑ", "ΓÖª", "ΓÖú", "ΓÖá", "Γ£ô", "Γ£ù", "Γê₧", "Γëê", "Γëá", "Γëñ", "ΓëÑ", "┬▒", "├╖", "├ù", "┬░", "╧Ç", "╬⌐", "╬╝", "╬ú", "Γé¼", "┬ú", "┬Ñ", "┬⌐", "┬«", "Γäó", "┬º", "┬╢", "ΓÇá"]
    ...
    def _toggle_checklist_char(self, cursor, char):
        ...
        if char == "ΓÿÉ":
             cursor.insertText("Γÿæ")
             return True
        elif char == "Γÿæ":
              cursor.insertText("ΓÿÉ")
              return True
        return False
    ...
    def _get_image_controls_html(self, res_name, is_whiteboard=False):
        ...
        html += f'<tr><td><a href="action://view/{res_name}" title="View External" style="{btn_style}">≡ƒû╝∩╕Å</a></td></tr>'
        html += f'<tr><td><a href="action://refresh/{res_name}" title="Refresh Image" style="{btn_style}">≡ƒöä</a></td></tr>'
        
        # Only show Edit button for whiteboard drawings
        if is_whiteboard:
            html += f'<tr><td><a href="action://edit/{res_name}" title="Edit" style="{btn_style}">Γ£Å∩╕Å</a></td></tr>'
            
        html += f'<tr><td><a href="action://delete/{res_name}" title="Delete" style="{del_style}">≡ƒùæ∩╕Å</a></td></tr>'
        html += '</table>'
    ...
    def insert_note_box(self):
        ...
        <table width="100%" border="1" cellpadding="10" cellspacing="0" style="border-collapse: collapse; border: 2px solid #007ACC; background-color: #f9f9f9;">
            <tr>
                <td style="background-color: #e3f2fd; font-weight: bold; color: #007ACC; border-bottom: 2px solid #007ACC;">Note: Γ£Æ∩╕Å</td>
                <td width="30" style="background-color: #e3f2fd; border-bottom: 2px solid #007ACC; text-align: right; vertical-align: middle;">
                    <a href="action://delete_level" style="text-decoration: none; color: #007ACC; font-weight: bold; font-size: 14px;">Γ£ò</a>
                </td>
            </tr>
            <tr>
                <td colspan="2"><br></td>
    ...
    def _insert_image_controls(self, cursor, res_name):
        ...
        <div align="center" style="margin-top: 8px; margin-bottom: 10px;">
            <a href="action://view/{res_name}" style="{edit_style}">≡ƒû╝∩╕Å View</a>
            <a href="action://refresh/{res_name}" style="{edit_style}">≡ƒöä Refresh</a>
            <a href="action://edit/{res_name}" style="{edit_style}">Γ£Å∩╕Å Edit</a>
            <a href="action://delete/{res_name}" style="{delete_style}">≡ƒùæ∩╕Å Delete</a>
        </div>
        '''
    ...
    def show_back_button(self, note_id, note_title):
        ...
        self.btn_back.setText(f"ΓåÉ Back to {display_title}")
        self.btn_back.show()
    ...
    def insert_checklist(self):
        ...
        if text.startswith("ΓÿÉ") or text.startswith("Γÿæ"):
            return
